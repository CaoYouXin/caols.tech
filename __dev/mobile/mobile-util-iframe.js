...
window.mobileUtil = (function (win, doc) {
    ...
    return {
        ...
        parse: function(theWin) {
            ...
            var metaEl = theWin.document.querySelector('meta[name="viewport"]'),
                metaCtt = (metaEl ? metaEl.content : '').replace(/\s*/g, '');

            var kvs = metaCtt.split(','), data = {metaEl: metaEl};
            ...
            data.width = parseInt(data.width) || 750;
        ...
        fixScreen: function () {
            ...
            //     }
            //
            //     data.metaEl.content = JSON.stringify(data).replace(/\s*/g, '').replace(/[{}"]/g, '').replace(/:/g, '=');
            // }

            var pData = this.parse(win.parent);

            console.log('this win', data, 'parent win', pData);
